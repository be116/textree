...
var load_log = function (id, next,  success, error) {
    ...
    post('log/load',
        ...
        (data) => {
            ...
            push_hist(data);
            success();
            show_log(); show_brunch();
        },
        (data) => { error(); alert('該当する文章は存在しません'); });
}
        ...
        var p = $("<button></button>", {
            ...
            on: {
                click: function (e) {
                    ...
                    load_log(data._id, data.brunch[$(this).val()].next, () => { }, () => { });
...
var save_log = function (brunch, text) {
    post('/log/save',
        ...
        , (data) => {
            ...
            load_log(d._id, d.brunch[d.brunch.length - 1].next, () => { }, () => { });
        }
        , () => { console.log('save log error!'); alert('この文章はこれ以上分岐できません') });
}
